...
public class EmployeeController {
    ...
    public String detail(@PathVariable("code") String code, Model model,
        ...
        if (employee != null) {
            ...
            // 対象データがない場合はフラッシュメッセージを設定し一覧画面へ戻る
    ...
    public String create(@ModelAttribute EmployeeForm form) {
        ...
        // @ModelAttributeの引数省略型を利用しているため、下記のように、モデル名はクラス名のローワーキャメルケースとなる
        // model.addAttribute("employeeForm", form);　→form.htmlへ引き継ぐモデル名となる
        // 更新画面表示・更新処理実行のメソッドにおいても上記と同様のモデル名とする
    ...
    public String edit(@PathVariable("code") String code, Model model,
        ...
        // GETメソッドでcode入力可能のため、URLでcodeを直入力された場合の、対象データの有無チェックを行う
        // 対象データを取得
        Employee entity = service.findByCode(code);
        // 対象データの有無確認
        if (entity != null) {
            // 対象データがある場合は処理を進める
            // EntityからFormへ変換
            EmployeeForm form = EmployeeHelper.convertForm(entity);
            // モデルに格納
            //　登録画面表示の@ModelAttribute引数省略型に合せ、モデル名はFormクラス名のローワーキャメルケースとする
